import type { ExperienceI } from "./experiences";
...
/**
 * The role James is in right now, if any.
 *
 * A role counts as current when it has no end date, or when its end date has
 * not arrived yet. The taskbar reads this rather than carrying its own copy of
 * the answer, so availability cannot drift from the roles on show in the
 * Experience window.
 */
export const currentRole = (now: Date = new Date()): ExperienceI | undefined =>
  experience.find((role) => typeof role.end === "string" || role.end > now);
