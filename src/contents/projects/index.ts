...
class ProjectsContent extends OSElement {
  ...
  /** Called once the first read of GitHub has settled, either way. */
  private arrived!: () => void;
  /** True once that has happened, and so once the window's cover has gone. */
  ...
  /*
   * The mark is the point of overriding this one.
   *
   * The default cover would say "Opening Projects…" over a P in a square, and
   * this window is not opening — it is on the network, waiting for somebody
   * else's API, which is a different kind of wait and takes a different amount
   * of time. GitHub's own mark says where the contents of this window come
   * from before a single repository has arrived.
   */
  readonly splash: SplashRequest = {
    icon: githubMark(),
    label: "Reading GitHub…"
  };
  ...
  private async fetch(force = false) {
    ...
    /*
     * Uncover the window, whichever way that went.
     *
     * A failure has as much to show as a success — the reason, and the button
     * that tries again — and both are behind the cover until this is called.
     * A later refresh calls it again to no effect; the cover is long gone.
     */
    this.uncovered = true;
    this.arrived();
  ...
  private paint() {
    ...
    if (this.phase === "loading" && !this.result) {
      /*
       * Only once the cover has gone.
       *
       * The window's splash says "Reading GitHub…" over this pane while the
       * first read is outstanding, and the glass it is made of is translucent
       * enough to show this line faintly through it — the same sentence, twice,
       * in two places. The line still earns its place afterwards: a read that
       * failed and is being tried again has no cover, and this is the only
       * thing saying anything is happening.
       */
      if (this.uncovered) this.body.appendChild(note("Reading GitHub…"));
      return;
    }
 ...
 *
 * An inline path costs no request and cannot fail, which matters for something
 * whose whole job is to be on screen while a request is outstanding — an icon
 * that has to be downloaded before it can say "this is being downloaded" is a
 * spinner that needs a spinner. It takes its colour from the text around it, so
 * it follows the theme without knowing anything about it.
 */
function githubMark(): HTMLElement {
  ...
  // The splash sizes its own mark; this only has to fill whatever it is given.
