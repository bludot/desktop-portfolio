...
var proto = {
    ...
    doStack: function (pixels, width, height) {
        var radius = this.radius;
        var i, p, np, r_sum, g_sum, b_sum, a_sum, r_out_sum, g_out_sum, b_out_sum, a_out_sum, r_in_sum, g_in_sum, b_in_sum, a_in_sum, pr, pg, pb, pa, rbs;
        // @ts-ignore
        var mul_sum = Blur.mul_table[radius], 
        // @ts-ignore
        shg_sum = Blur.shg_table[radius];
        var widthMinus1 = width - 1, heightMinus1 = height - 1, radiusPlus1 = radius + 1, sumFactor = (radiusPlus1 * (radiusPlus1 + 1)) / 2, div = radius + radius + 1, w4 = width << 2;
        // @ts-ignore
        var stackStart = Blur.newBlurStack(), stackEnd = null, stackIn = null, stackOut = null, stack = stackStart;
        for (i = 1; i < div; i++) {
            // @ts-ignore
            ...
            for (pix1 = 0; pix1 < count1;) {
                ...
                for (pix2 = 0; pix2 < count2; pix2++) {
                    if (isH) {
                        np =
                            (jump +
                                ((np = pix2 + radiusPlus1) < widthMinus1 ? np : widthMinus1)) <<
                                2;
                    }
                    else {
                        np =
                            (pix1 +
                                ((np = pix2 + radiusPlus1) < heightMinus1 ? np : heightMinus1) *
                                    width) <<
                                2;
                    }
    ...
    blurRGBA: function (canvas, outCanvas, ext) {
        ...
        var sx = 0, sy = 0, width = canvas.width, height = canvas.height;
