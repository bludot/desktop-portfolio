...
/*
 * A window that is moving gives up its live blur for the length of the gesture.
 * A backdrop filter re-samples whatever is behind it, so on something that
 * moves the compositor redoes a 30px blur every frame over a backdrop that
 * never holds still — see `theme/global`. Nothing is put in its place: the
 * window already paints its own translucent fill.
 */
describe('the blur, while a window is being moved', () => {
  const press = (win: any) =>
    win.mousedown({
      pageX: 10,
      pageY: 10,
  ...
  const moving = (win: any) => win.element.classList.contains('is-moving')

  it('is left alone by a press that has not moved yet', () => {
    const win = makeWindow()
    press(win)

    expect(moving(win)).toBe(false)
  })

  it('is dropped once the pointer actually moves', () => {
    const win = makeWindow()
    press(win)
    win.mousemove({ pageX: 40, pageY: 30, preventDefault() {} } as MouseEvent)

    expect(moving(win)).toBe(true)
  })

  it('comes back when the pointer is released', () => {
    const win = makeWindow()
    press(win)
    win.mousemove({ pageX: 40, pageY: 30, preventDefault() {} } as MouseEvent)
    win.mouseup({ pageX: 40, pageY: 30 } as MouseEvent)
